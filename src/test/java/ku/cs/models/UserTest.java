package ku.cs.models;

import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

class UserTest {

    @Test
    @DisplayName("Password should not store in plain text")
    public void testPasswordIsNotStoreInPlainText() {
        User user = new User("user01", "plain-p@ssw0rd");
        String actual = user.getPassword();
        String unexpected = "plain-p@ssw0rd";
        assertNotEquals(unexpected, actual);
    }

    @Test
    @DisplayName("Password should be verified by plain text")
    public void testPasswordShouldBeVerified() {
        User user = new User("user01", "plain-p@ssw0rd");
        boolean actual = user.validatePassword("plain-p@ssw0rd");
        assertTrue(actual);
    }

    @Test
    @DisplayName("Password should not match if incorrect")
    public void testIncorrectPasswordValidation() {
        User user = new User("user01", "plain-p@ssw0rd");
        boolean actual = user.validatePassword("incorrect-password");
        assertFalse(actual);
    }

    @Test
    @DisplayName("Set new password and verify it")
    public void testSetNewPasswordAndVerify() {
        User user = new User("user01", "initial-password");
        String newPassword = "new-p@ssw0rd";
        user.setPassword(newPassword);
        boolean actual = user.validatePassword(newPassword);
        assertTrue(actual);
    }

    @Test
    @DisplayName("Username should match")
    public void testUsernameMatch() {
        User user = new User("user01", "plain-p@ssw0rd");
        boolean actual = user.isUsername("user01");
        assertTrue(actual);
    }

    @Test
    @DisplayName("Username should not match if incorrect")
    public void testIncorrectUsername() {
        User user = new User("user01", "plain-p@ssw0rd");
        boolean actual = user.isUsername("incorrectUsername");
        assertFalse(actual);
    }
}
